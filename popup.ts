...
async function searchMemories(query: string): Promise<SearchResult[]> {
  ...
  rewriteQuery = await queryRewriteAgent(query, settings?.apiKey);

  // 2. Try semantic search if embeddings are available
  ...
  const lowercaseQuery = rewriteQuery.toLowerCase();
  return memories
    .filter(
